    ...
    <ProtectedRoute>
      <div className="min-h-[calc(100vh-4rem)] bg-zinc-950">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white">Hola, {usuario ? usuario.nombre.split(' ')[0] : 'jugador'} ⚽</h1>
            <p className="text-zinc-400 mt-1">{new Date().toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
          </div>

          {cargando ? <Loading texto="Cargando tu dashboard..." /> : (
            <>
              {alertas.length > 0 && (
                <div className="mb-6 space-y-2">
                  {alertas.map((alerta, i) => (
                    <div key={i} className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 flex items-center justify-between gap-4">
                      ...
                      <button onClick={() => router.push(`/grupos/${alerta.grupoId}`)} className="text-xs font-semibold px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black transition flex-shrink-0">Predecir →</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4"><p className="text-2xl font-bold text-white">{grupos.length}</p><p className="text-xs text-zinc-500 mt-1">Grupos activos</p></div>
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4"><p className="text-2xl font-bold text-emerald-400">{alertas.length}</p><p className="text-xs text-zinc-500 mt-1">Pendientes</p></div>
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 cursor-pointer hover:border-zinc-600 transition" onClick={() => router.push('/partidos')}><p className="text-2xl">⚽</p><p className="text-xs text-zinc-500 mt-1">Ver partidos</p></div>
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 cursor-pointer hover:border-zinc-600 transition" onClick={() => router.push('/perfil')}><p className="text-2xl">👤</p><p className="text-xs text-zinc-500 mt-1">Mi perfil</p></div>
              </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl divide-y divide-zinc-800">
                  {grupos.map((grupo, i) => (
                    <div key={grupo._id} onClick={() => router.push(`/grupos/${grupo._id}`)} className="flex items-center gap-3 p-4 cursor-pointer hover:bg-zinc-800/50 transition">
                      <div className={`w-10 h-10 rounded-xl ${colores[i % colores.length]} flex items-center justify-center text-white text-sm font-bold flex-shrink-0`}>{iniciales(grupo.nombre)}</div>
